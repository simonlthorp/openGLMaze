...
struct SpotLight {
    ...
    lowp vec3 AmbientIntensity;
    lowp vec3 DiffuseIntensity;
    lowp vec3 SpecularIntensity;
...
void main(void) {
    ...
    //lowp vec4 test = u_MatColor * texture2D(u_Texture, frag_TexCoord) * vec4((AmbientColor + DiffuseColor), 1.0);
    
    //gl_FragColor = test + vec4(SpecularColor, 1.0);
    
    gl_FragColor = u_MatColor * texture2D(u_Texture, frag_TexCoord) * vec4((AmbientColor + DiffuseColor + SpecularColor), 1.0);
    //gl_FragColor = u_MatColor * texture2D(u_Texture, frag_TexCoord) * vec4((AmbientColor + DiffuseColor), 1.0);
}
